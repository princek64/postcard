...
export type Postcard = { id:string; sender_id:string|null; recipient_name:string; recipient_contact:string; photo_url:string; filter_type:FilterType; caption:string; note:string; location_name:string; lat:number; lng:number; created_at:string; opened_at:string|null };
export type NewPostcard = Omit<Postcard, 'id' | 'created_at' | 'opened_at'> & { opened_at?: string | null };
